# 1 version of programm without c, str, cg
# import math
# import os
...
#         self.counter = 1 #counter of records in file
#         if not os.path.exists(self.results_file):
#             with open(self.results_file, 'w', encoding='utf-8') as f:
#                 f.write("Результаты геодезических расчетов\n\n")
#     def save_to_file(self, data): # function to save results
#         with open(self.results_file, 'a', encoding='utf-8') as f: #"a" add to the end of the file
#             f.write(f"{self.counter}. {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}\n")
#             f.write(data + "\n\n")
#             self.counter += 1
#     def degrees_to_dms(self, degrees):
#         """Конвертация десятичных градусов в градусы, минуты, секунды"""
#         d = int(degrees)
#         remainder = (degrees - d) * 60 #convert fractional part into minutes
#         m = int(remainder)
...
#             print("\n" + result)
#             self.save_to_file(
#                 f"Прямая задача:\nИсходные данные: x1={x1}, y1={y1}, α={alpha_deg}°, L={L}\nРезультат: {result}")
#             return x2, y2
#         except ValueError as e:
#             print(f"Ошибка ввода данных: {e}. Пожалуйста, вводите числовые значения.")# "e" output specific message about mistake
#             return None # return empty meaning
...
#             if delta_x == 0:
#                 alpha = 90 if delta_y > 0 else 270
#             else:
#                 alpha_rad = math.atan2(delta_y, delta_x)  # Автоматически определяет четверть
#                 alpha = math.degrees(alpha_rad) % 360  # Приводим к диапазону [0, 360)
#             d, m, s = self.degrees_to_dms(alpha)
#             result = (f"Расстояние между точками: L = {L:.3f}\n"
#                       f"Дирекционный угол: α = {d}° {m}' {s:.2f}\"\n"
#                       f"Приращения координат:\n"
#                       f"Δx = {delta_x:.3f}\n"
#                       f"Δy = {delta_y:.3f}")
#             print("\n" + result)
#             self.save_to_file(
#                 f"Обратная задача:\nИсходные данные: x1={x1}, y1={y1}, x2={x2}, y2={y2}\nРезультат: {result}")
#             return L, alpha
#         except ValueError as e:
#             print(f"Ошибка ввода данных: {e}. Пожалуйста, вводите числовые значения.")
#             return None
# def main():
#     calculator = GeodeticCalculator()
#     while True:
#         print("\nГлавное меню:")
#         print("1. Решение прямой геодезической задачи")
#         print("2. Решение обратной геодезической задачи")
#         print("3. Выход")
#         choice = input("Выберите действие (1-3): ").strip()
#         if choice == '1':
#             calculator.direct_problem()
#         elif choice == '2':
#             calculator.inverse_problem()  # Была опечатка в названии метода
#         elif choice == '3':
